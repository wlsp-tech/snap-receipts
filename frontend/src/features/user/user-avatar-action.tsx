...
const UserAvatarAction = ({nameOfUser, onLogout, btnClassName}: UserAvatarProps) => {
    ...
    return (
        <>
            ...
            {open && (
                <div className={cn(
                    "absolute top-18 border-indigo-200 border w-60 rounded-xl shadow-lg z-30 p-4 bg-accent hover:bg-accent hover:text-accent-foreground transition-all ease-in-out",
                )}>
                    <Button
                        variant="destructive"
                        onClick={() => {
